...
for (i in variavs) {
  ...
  wrf_temporary <- disaggregate(wrf_temporary, fact=c(3, 3), method='bilinear')
...
lat <- seq(wrf_topo@extent@ymin, wrf_topo@extent@ymax, by = res(wrf_topo)[2])
lon <- seq(wrf_topo@extent@xmin, wrf_topo@extent@xmax, by = res(wrf_topo)[1])
